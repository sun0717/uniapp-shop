...
import { getMemberOrderByIdAPI, getMemberOrderConsignmentByIdAPI, putMemberOrderReceiptByIdAPI, getMemberOrderLogisticsByIdAPI, getMemberOrderCancelByIdAPI } from '@/services/order'
...
const onOrderCancel = () => {
    ...
    uni.showModal({
        ...
        success: async (success) => {
            if (success.confirm) {
                await getMemberOrderCancelByIdAPI(query.id, { cancelReason: 'no way' })
        ...
        <template v-if="order">
            ...
            <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
                ...
                <template v-if="order?.orderState === OrderState.DaiFuKuan">
                    ...
                    <view class="tips">
                        <text class="money">应付金额: ¥ {{ order.payMoney }}.00</text>
                        <text class="time">支付剩余</text>
                        <!-- 不显示天数 -->
                        <uni-countdown :show-day="false" :second="order.countdown" color="#fff" :showColon="false"
                ...
                <template v-else>
                    ...
                    <view class="button-group">
                        ...
                        <view v-if="isDev && order.orderState == OrderState.DaiFaHuo" class="button" @tap="onOrderSend">
                            模拟发货 </view>
                        <view v-if="isDev && order.orderState == OrderState.DaiShouHuo" class="button primary"
                            @tap="onOrderConfirm">
                            确认收货 </view>
                    </view>
                </template>
            ...
            <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
                ...
                <template v-else>
                    ...
                    <view class="button delete" v-if="order.orderState >= OrderState.DaiPingJia" @tap="onOrderCancel"> 删除订单
                    </view>
                </template>
            </view>
        </template>
